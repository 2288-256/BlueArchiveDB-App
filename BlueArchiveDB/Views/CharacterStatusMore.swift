...
class CharacterStatusMore: UIViewController
{
	...
	override func viewDidLoad()
	{
		...
		for i in 1 ... 3
		{
			let equipmentView = view.viewWithTag(i) as! UIView
	...
	@IBAction func TierSliderChanged(_ sender: UISlider)
	{
		...
		let equipmentView = view.viewWithTag(senderTag / 10) as! UIView
	...
	func UpdateWeaponLevel(level: Int, equipmentEffectLabel: UILabel)
	{
		...
		let equipmentView = view.viewWithTag(4) as! UIView
		var WeaponImage = view.viewWithTag((4 * 100) + 3) as! UIImageView
		if nowWeaponLevel != 0
		{
	...
	func LabelSetUp(level: Int, noWeaponLevel _: Bool = true)
	{
		addBonus.removeAll()
		var Equipment1 = view.viewWithTag(103) as! UIImageView
		var Equipment2 = view.viewWithTag(203) as! UIImageView
		var Equipment3 = view.viewWithTag(303) as! UIImageView
		...
		for i in 1 ... StatusList.count
		{
			var beforeLabel = view.viewWithTag((i * 1000) + 2) as! UILabel
			var addLabel = view.viewWithTag((i * 1000) + 3) as! UILabel
			var addPercentLabel = view.viewWithTag((i * 1000) + 4) as! UILabel
			var afterLabel = view.viewWithTag((i * 1000) + 5) as! UILabel
	...
	override func touchesBegan(_ touches: Set<UITouch>, with _: UIEvent?)
	{
		...
		let popUpView: UIView = view.viewWithTag(100)! as UIView

		if !popUpView.frame.contains(tapLocation)
		{
			loadItem()
			var EquipmentTier: [Int: String] = [:]
			for i in 1 ... 3
			{
				let equipmentView = view.viewWithTag(i) as! UIView
				let equipmentTierSlider = equipmentView.viewWithTag(i * 10) as! UISlider
